...
export function ServicesSection({
  showAllLink = true,
  className,
  headingPrefix = "Software Development Services We",
  headingAccent = "Engineer",
  description = "Custom software, scalable web platforms, mobile apps, and AI workflow automation built to solve your operational bottlenecks.",
}: ServicesSectionProps = {}) {
  return (
    <section id="services" className={cn("relative overflow-hidden py-16 lg:py-24", className)}>
      <ServicesBg />
        ...
        >
          ...
          <motion.div variants={fadeUp} className="mb-14 text-center">
            ...
            <h2 className="mx-auto max-w-3xl text-3xl font-bold leading-[1.15] tracking-tight text-foreground sm:text-4xl lg:text-5xl">
              {headingPrefix} <span className="font-serif italic text-foreground">{headingAccent}</span>
            </h2>
          ...
          {/* ── All Services Link (only when not on the services hub itself) ── */}
          {showAllLink && (
            <motion.div variants={fadeUp} className="mt-12 text-center">
              <Link
                href="/services"
                className="inline-flex items-center gap-2 rounded-full border border-border/80 bg-background/60 px-5 py-2.5 text-xs font-semibold text-foreground backdrop-blur-sm transition-all duration-200 hover:border-foreground/30 hover:bg-muted/60"
              >
                <span>Explore All Software Development Services &amp; Capabilities</span>
